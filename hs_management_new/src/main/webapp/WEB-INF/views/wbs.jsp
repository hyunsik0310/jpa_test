...
<h1><a href="getwbsinfo?date=${date}&u_no=${wbsinfo.u_no}&mod=pre">&lt;</a> ${date} <a href="getwbsinfo?date=${date}&u_no=${wbsinfo.u_no}&mod=next">&gt;</a></h1>
<table border=1>
   <tr style="background-color: lightblue">
      ...
      <form:form modelAttribute="wbsinfo" action="updatewbs">
	      <c:forEach items = "${wbsinfo.wbslist }" var= "wbs" varStatus="i">
			<tr>
			<td>${wbs.date} ${wbs.day }</td>
				<form:hidden path="wbslist[${i.index}].date"/>
	        <td>
	         	<form:select path = "wbslist[${i.index}].start_h">
	         		<form:options items = "${hour }"/>
	         	</form:select>
	         :
	         ...
	         <td>
	         	<form:select path = "wbslist[${i.index}].end_h">
	         		<form:options items = "${hour }"/>
	         	</form:select>
	         :
	         ...
	         <td>${wbs.total_h}</td>
	         <td>
	         	<form:select path = "wbslist[${i.index}].vacation_type">
	         		<form:options items = "${vacationCode}"/>
				</form:select>
	         </td>
	         <td><form:input size="6px" path = "wbslist[${i.index}].memo"/></td>
	      </tr>
	      </c:forEach>
